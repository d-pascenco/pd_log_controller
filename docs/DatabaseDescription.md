...
PostgreSQL в проекте используется как основное хранилище логов.
...
Это для того, чтобы потом поменять PostgreSQL на другую БД (MySQL, SQLite),
...
```
Берём строку подключения из переменной окружения (в docker-compose).
Если переменной нет, то используется значение по умолчанию.
...
```
Пул сессий. Каждый раз когда нужно что-то сделать с БД
